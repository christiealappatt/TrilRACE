...
template <typename packtype> class kernels {
private:
...
public:
  kernels(preProcess<packtype> *_preProcess_,
          Teuchos::RCP<CrsMatrixType> _M_ = Teuchos::null)
      : pre_process(_preProcess_), A(NULL), L(NULL), U(NULL), M(NULL), ce(NULL),
        workspacePolyPrecon(NULL), workspacePolyPrecon2(NULL),
        workspaceGmresSstep(NULL), workspacePrecon(NULL), precType("NONE"),
        precSide("NONE"), subPow(1), gamma(1), lambdaMin(std::nan("")),
        lambdaMax(std::nan("")), eigRatio(std::nan("")), paramUptodate(false) {
   ...
   * kernels(): A(NULL), L(NULL), U(NULL), M(NULL), ce(NULL),
  workspacePolyPrecon(NULL), workspacePolyPrecon2(NULL),
  workspaceGmresSstep(NULL), workspacePrecon(NULL), precType("NONE"),
  precSide("NONE"), subPow(1), gamma(1), lambdaMin(std::nan("")),
  lambdaMax(std::nan("")), eigRatio(std::nan(""))
  {
  }*/
  ...
  void init(Teuchos::RCP<CrsMatrixType> _M_ = Teuchos::null) {
    ...
    // std::transform(precon_side.begin(), precon_side.end(),
    // precon_side.begin(), ::toupper);
  ...
  std::string getPrecType() { return precType; }
  void createPrecon();
...
#define TrilinosRACE_MPK_KERNEL_BODY(_NAME_, ZERO_OUT_CODE, ...)               \
  {                                                                            \
    ...
    int race_power_id = ce->registerFunction(                                  \
        &RACE_##_NAME_##_KERNEL<packtype>, voidArg, -1, subPow);               \
    if (forceSubPower > 0) {                                                   \
    ...
    {                                                                          \
      ...
      for (int block = 0; block < powBlocks; ++block) {                        \
        RACE_##_NAME_##_setOffset(                                             \
            block * bestPow); /*offset array access for the iteration*/        \
        ce->executeFunction(race_power_id);                                    \
        ZERO_OUT_CODE;                                                         \
      }                                                                        \
      if (remPow > 0) {                                                        \
        RACE_##_NAME_##_setOffset(                                             \
            powBlocks * bestPow); /*offset array access for the iteration*/    \
  ...
  // MPK performs for i=1,...,power {x[i+1]=beta*x[i+1]+alpha*A*x[i]}; where
  // x[i] is a vector of length nrows. Assumes x[0] is the initial input vector
  ...
  int PreconKernel(int power, const vec_type &b, vec_type &x, bool fwdDir) {
    ...
    int tunedPow =
        1; // no tuning for this kernel as it is normally called for power=1
    int forceSubPower = 1; // enforce subpower to 1

    int innerIter = subPow - 1;
    if (power > 0) {
      if (((precType == "GAUSS-SEIDEL") ||
           (precType == "JACOBI-GAUSS-SEIDEL")) ||
      ...
      if (precType == "TWO-STEP-GAUSS-SEIDEL") {
        ...
        TrilinosRACE_MPK_KERNEL_BODY(Precon, , A, L, U, &b_arr, &x_arr,
                                     initVecIsZero, gamma, 0, innerIter,
                                     precType, fwdDir, &preconTmp);
      } else {
        TrilinosRACE_MPK_KERNEL_BODY(Precon, , A, L, U, &b_arr, &x_arr,
                                     initVecIsZero, gamma, 0, innerIter,
                                     precType, fwdDir, NULL);
      }
    }
    return tunedPow;
  }
  ...
  // false
  bool allocateVecWorkspace(vec_type *&workspace,
                            const Teuchos::RCP<const map_type> &map, int ncols,
                            bool zeroOut = true) {
    ...
    // Vinay Edit: For now this seemes not necessary and it is giving a small
    // speedup
    //  else if (zeroOut) {
  ...
  int MPK_GmresSstepKernel(int power, int iter, vec_type &x,
                           std::vector<complex_type> theta, int tunedPow) {
    if (!paramUptodate) {
      setupParams();
    }
    ...
    // for preconditioner we have to create temporary work space to hold
    // tunedPow columns iterations
    if ((precType != "NONE") && (precType != "JACOBI")) {
      if (precType == "TWO-STEP-GAUSS-SEIDEL") {
    ...
    if (power > 0) {
      ...
      if (needAllocation) {
        ...
        TrilinosRACE_MPK_KERNEL_BODY(GmresSstepKernel, , A, L, U, &x_arr,
                                     initVecIsZero, theta, gamma, 0, iter,
                                     innerIter, precType, precSide, &preconTmp);
      } else {
        TrilinosRACE_MPK_KERNEL_BODY(GmresSstepKernel, , A, L, U, &x_arr,
                                     initVecIsZero, theta, gamma, 0, iter,
                                     innerIter, precType, precSide, NULL);
      }
    }
    return tunedPow;
  }
  ...
  int MPK_GmresPolyPreconKernel(int power, vec_type &prod, vec_type &y,
                                std::vector<complex_type> theta, int tunedPow) {
    if (!paramUptodate) {
      setupParams();
    }
    ...
    allocCol = std::max(2, allocCol); // ensure minimum of 2, due to access of
                                      // prev_offset in case of theta_i != 0
    // last argument is false, since we disable zeroOut as it is not
    // required here
    allocateVecWorkspace(workspacePolyPrecon, prod.getMap(), allocCol + 1,
                         true);

    // set first vector to prod
    // Teuchos::Range1D index(0, 0);
    // Teuchos::RCP<vec_type> prod_subview =
    // workspacePolyPrecon->subViewNonConst (index);
    Teuchos::RCP<vec_type> prod_subview =
        workspacePolyPrecon->getVectorNonConst(0);
    ...
    if ((precType != "NONE") && (precType != "JACOBI")) {
      if (precType == "TWO-STEP-GAUSS-SEIDEL") {
        allocateVecWorkspace(workspacePolyPrecon2, prod.getMap(), innerIter,
                             true);
      } else {
        allocateVecWorkspace(workspacePolyPrecon2, prod.getMap(), allocCol + 1,
                             true);
      }
      needAllocation = true;
    }
    ...
    if (power > 0) {
      ...
      if (needAllocation) {
        ...
        TrilinosRACE_MPK_KERNEL_BODY(
            GmresPolyPreconKernel, workspacePolyPrecon2->putScalar(0);
            , A, L, U, &x_arr, &y_arr, initVecIsZero, theta, gamma, 0, power,
            allocCol, innerIter, precType, precSide, &preconTmp);
      } else {
        TrilinosRACE_MPK_KERNEL_BODY(GmresPolyPreconKernel, , A, L, U, &x_arr,
                                     &y_arr, initVecIsZero, theta, gamma, 0,
                                     power, allocCol, innerIter, precType,
                                     precSide, NULL);
      }
    }
  ...
  int MPK_MGSmootherKernel(int power, vec_type &x, vec_type &b, bool zeroGuess,
                           bool fwdDir, int tunedPow) {
    if (!paramUptodate) {
      setupParams();
    }
    // for prod we have to create temporary work space to hold tunedPow columns
    ...
    if (precType == "TWO-STEP-GAUSS-SEIDEL") {
      ...
      printf("Error: only Two-stage Gauss-Seidel and Chebyshev implemented "
             "currently for MG smoother\n");
    }
    if (!zeroGuess && needAllocation) {
      ...
      // Teuchos::RCP<vec_type> prod_subview =
      // workspacePolyPrecon->subViewNonConst (index);
      Teuchos::RCP<vec_type> x_subview =
          workspacePolyPrecon->getVectorNonConst(0);
    ...
    if (power > 0) {
      ...
      if (needAllocation) {
        ...
        TrilinosRACE_MPK_KERNEL_BODY(MGSmootherKernel, ;
                                     , A, L, U, &x_arr, &b_arr, NULL,
                                     initVecIsZero, gamma, scaleCoeff,
                                     scale2Coeff, 0, power, allocCol, innerIter,
                                     precType, fwdDir, &preconTmp);
      } else {

        printf("Error: only Two-stage Gauss-Seidel implemented currently for "
               "smoother\n");
        // TrilinosRACE_MPK_KERNEL_BODY(GmresPolyPreconKernel, , A, L, U,
        // &x_arr, &y_arr, initVecIsZero, theta, gamma, 0, power, allocCol,
        // innerIter, precType, NULL);
      }
    }
  ...
  // MPK_MGSmoother performs 'n' sweeps of the MG smoother
  int MPK_MGSmootherKernel(int power, vec_type &x, vec_type &b, vec_type &r,
                           bool zeroGuess, bool fwdDir, int tunedPow) {
    if (!paramUptodate) {
      setupParams();
    }
    ...
    if (power > 0) {
      ...
      if (needAllocation) {
        ...
        TrilinosRACE_MPK_KERNEL_BODY(MGSmootherKernel_w_Residual, ;
                                     , A, L, U, &x_arr, &b_arr, &r_arr,
                                     initVecIsZero, gamma, scaleCoeff,
                                     scale2Coeff, 0, power, allocCol, innerIter,
                                     precType, fwdDir, &preconTmp);
      } else {
