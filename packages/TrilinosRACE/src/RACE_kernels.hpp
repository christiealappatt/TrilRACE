...
namespace RACE {
    ...
    template <typename packtype>
        class kernels
        {
            ...
            public:
            kernels(RACE::Interface *_ce_, Teuchos::RCP<CrsMatrixType> _A_, std::string _precType_="NONE", Teuchos::RCP<CrsMatrixType> _M_=Teuchos::null): A(NULL), M(NULL), ce(NULL), workspacePolyPrecon(NULL)
            {
                init(_ce_, _A_, _precType_, _M_);
            }
            kernels(): A(NULL), M(NULL), ce(NULL), workspacePolyPrecon(NULL)
            {
            }
            void init(RACE::Interface *_ce_, Teuchos::RCP<CrsMatrixType> _A_, std::string _precType_="NONE", Teuchos::RCP<CrsMatrixType> _M_=Teuchos::null)
            {
                ce = _ce_;
                if(ce == NULL)
                {
...
\
            ...
            /*it and I am restting it next*/\
            int race_power_id = ce->registerFunction(&RACE_ ## _NAME_ ## _KERNEL<packtype>, voidArg, -1);\
\
...
\
            ...
            int MPK_GmresSstepKernel(int power, vec_type &x, std::vector<complex_type> theta, int tunedPow)
            {
                if(power > 0)
                {
                    array_type x_arr = x.get2dViewNonConst();
                    ...
                    TrilinosRACE_MPK_KERNEL_BODY(GmresSstepKernel, A, &x_arr, theta, 0);
                }
                return tunedPow;
            }
            ...
            //false
            bool allocateVecWorkspace(vec_type* &workspace, const Teuchos::RCP<const map_type>& map, int ncols)
            {
                ...
                if(newAllocate)
                {
                    workspace = new vec_type(map, ncols);
                }
            }
            ...
            int MPK_GmresPolyPreconKernel(int power, vec_type &prod, vec_type &y, std::vector<complex_type> theta, int tunedPow)
            {
                ...
                if(power > 0)
                {
                    ...
                    TrilinosRACE_MPK_KERNEL_BODY(GmresPolyPreconKernel, A, &x_arr, &y_arr, theta, 0, power, allocCol);
                }
