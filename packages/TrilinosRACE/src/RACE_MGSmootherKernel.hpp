...
// convenience macros
#define RACE_ENCODE_TO_VOID_MGSmootherKernel(                                  \
    A_en, L_en, U_en, x_en, b_en, r_en, initVecIsZero_en, innerDamping_en,     \
    scaleCoeff_en, scale2Coeff_en, arr_offset_en, maxSteps_en, tunedPower_en,  \
    totSubPower_en, preconType_en, fwdDir_en, gmresPolyPreconTmp_en)           \
...
#define RACE_DELETE_ARG_MGSmootherKernel() delete arg_encode;
...
#define RACE_ENCODE_TO_VOID_MGSmootherKernel_w_Residual(                       \
    A_en, L_en, U_en, x_en, b_en, r_en, initVecIsZero_en, innerDamping_en,     \
    scaleCoeff_en, scale2Coeff_en, arr_offset_en, maxSteps_en, tunedPower_en,  \
    totSubPower_en, preconType_en, fwdDir_en, gmresPolyPreconTmp_en)           \
  RACE_ENCODE_TO_VOID_MGSmootherKernel(                                        \
      A_en, L_en, U_en, x_en, b_en, r_en, initVecIsZero_en, innerDamping_en,   \
      scaleCoeff, scale2Coeff, arr_offset_en, maxSteps_en, tunedPower_en,      \
      totSubPower_en, preconType_en, fwdDir_en, gmresPolyPreconTmp_en)
  ...
  if (next_pow == 1) {                                                         \
    ...
    if (initVecIsZero) {                                                       \
      for (LocalOrdinal row = start; row < end; ++row) {                       \
        (*preconTmp)[2][row] = scale * A->invDiag[row] *                       \
      ...
      for (LocalOrdinal row = start; row < end; ++row) {                       \
        ...
        (*preconTmp)[2][row] = scale * A->invDiag[row] *                       \
                               ((*preconInArray)[precon_cur_offset][row] -     \
                                tmp); /*W is stored in (*preconTmp)[2]*/       \
        (*preconOutArray)[precon_next_offset][row] =                           \
            (*preconTmp)[cur_pow_w_wrap][row] + (*preconTmp)[2][row];          \
      }                                                                        \
    }                                                                          \
  } else {                                                                     \
    double scale = (*scaleCoeff)[cur_pow];                                     \
    ...
    for (LocalOrdinal row = start; row < end; ++row) {                         \
      ...
      (*preconTmp)[2][row] =                                                   \
          scale * A->invDiag[row] *                                            \
              ((*preconInArray)[precon_cur_offset][row] - tmp) +               \
          scale2 * (*preconTmp)[2][row]; /*W is stored in (*preconTmp)[2]*/    \
      (*preconOutArray)[precon_next_offset][row] =                             \
          (*preconTmp)[cur_pow_w_wrap][row] + (*preconTmp)[2][row];            \
    }                                                                          \
  }
...
#define GENERATE_KERNEL_MGSmootherKernel_w_RESIDUAL(_Dir_)                     \
  template <typename packtype>                                                 \
  inline void RACE_MGSmootherKernel_##_Dir_##_KERNEL_w_RESIDUAL(               \
...
#define GENERATE_KERNEL_MGSmootherKernel(_Dir_)                                \
  template <typename packtype>                                                 \
  inline void RACE_MGSmootherKernel_##_Dir_##_KERNEL(                          \
      int start, int end, int pow, int subPow, int numa_domain, void *args) {  \
...
// dispatcher function
template <typename packtype>
inline void RACE_MGSmootherKernel_w_Residual_KERNEL(int start, int end, int pow,
                                                    int subPow, int numa_domain,
                                                    void *args) {
  using arg_type = kernelArgMGSmoother<packtype>;
  ...
  if (arg_decode->fwdDir) {
    RACE_MGSmootherKernel_FWD_KERNEL_w_RESIDUAL<packtype>(
        start, end, pow, subPow, numa_domain, args);
  } else {
    RACE_MGSmootherKernel_BWD_KERNEL_w_RESIDUAL<packtype>(
        start, end, pow, subPow, numa_domain, args);
  }
}

template <typename packtype>
inline void RACE_MGSmootherKernel_KERNEL(int start, int end, int pow,
                                         int subPow, int numa_domain,
                                         void *args) {
  using arg_type = kernelArgMGSmoother<packtype>;
  arg_type *arg_decode = (arg_type *)args;
  ...
  // only two-step implemented now
  if (arg_decode->fwdDir) {
    RACE_MGSmootherKernel_FWD_KERNEL<packtype>(start, end, pow, subPow,
                                               numa_domain, args);
  } else {
    RACE_MGSmootherKernel_BWD_KERNEL<packtype>(start, end, pow, subPow,
                                               numa_domain, args);
  }
}
