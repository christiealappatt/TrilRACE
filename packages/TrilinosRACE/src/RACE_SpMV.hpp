...
// i.e., oif using SpMV then use SpMV (and not SPMV) all over as seen in this
// template
namespace RACE {

template <typename packtype> struct kernelArgSpMV {
  // single vector case only implemented, since Tpetra solvers use them and not
  // multivector
...
#define RACE_DELETE_ARG_SpMV() delete arg_encode;
...
// Removed the _Pragma("omp simd simdlen(VECTOR_LENGTH) reduction(+:tmp)")
// clang-format off
...
inline void RACE_SpMV_KERNEL(int start, int end, int pow, int subPow,
