...
// convenience macros
#define RACE_ENCODE_TO_VOID_Precon(                                            \
    A_en, L_en, U_en, b_en, x_en, initVecIsZero_en, innerDamping_en,           \
    arr_offset_en, totSubPower_en, preconType_en, fwdDir_en, preconTmp_en)     \
...
#define RACE_DELETE_ARG_Precon() delete arg_encode;
...
#define BASE_PRECON_JACOBI_w_SpMV_RIGHT_KERNEL_IN_ROW(_A_, _x_in_vec_,         \
                                                      _x_out_vec_)             \
  Scalar tmp = 0;                                                              \
  _Pragma("omp simd simdlen(VECTOR_LENGTH) reduction(+:tmp)")                  \
      _Pragma("nounroll") for (int idx = (int)_A_->rowPtr[row];                \
                               idx < (int)_A_->rowPtr[row + 1]; ++idx) {       \
    int col_idx = _A_->col[idx];                                               \
    tmp +=                                                                     \
        (_A_->val[idx]) * (_A_->invDiag[col_idx]) * ((_x_in_vec_)[col_idx]);   \
  }

// For Jacobi the preconditioner and SpMV are fused together
// therefore we have to distinguish right and left precon here
#define BASE_PRECON_JACOBI_w_SpMV_LEFT_KERNEL_IN_ROW(_A_, _x_in_vec_,          \
                                                     _x_out_vec_)              \
  Scalar tmp = 0;                                                              \
  _Pragma("omp simd simdlen(VECTOR_LENGTH) reduction(+:tmp)")                  \
      _Pragma("nounroll") for (int idx = (int)_A_->rowPtr[row];                \
...
#define BASE_PRECON_JACOBI_GAUSS_SEIDEL_KERNEL_IN_ROW(_A_, _b_vec_,            \
                                                      _xInit_vec_)             \
  Scalar tmp = 0;                                                              \
  _Pragma("omp simd simdlen(VECTOR_LENGTH) reduction(+:tmp)")                  \
      _Pragma("nounroll") for (int idx = (int)_A_->rowPtr[row];                \
                               idx < (int)_A_->rowPtr[row + 1]; ++idx) {       \
    int col_idx = _A_->col[idx];                                               \
    Scalar value = ((col_idx >= start) && (col_idx < row))                     \
                       ? (_A_->val[idx] * ((_xInit_vec_)[col_idx]))            \
                       : 0;                                                    \
    tmp = tmp + value;                                                         \
  }                                                                            \
  tmp = _xInit_vec_[row] + (_A_->invDiag[row]) * (_b_vec_[row] - tmp);
...
#define BASE_PRECON_MPK_splitUL(_scale_, ...)                                  \
  for (LocalOrdinal row = start; row < end; ++row) {                           \
    BASE_SpMV_KERNEL_IN_ROW(U, (*mpkInArray)[mpk_cur_offset]);                 \
    (*mpkOutArray)[mpk_next_offset][row] =                                     \
        tmp + /*U->diag[row]*/ 1 * (*mpkInArray)[mpk_cur_offset][row];         \
  }                                                                            \
  for (LocalOrdinal row = start; row < end; ++row) {                           \
    BASE_SpMV_KERNEL_IN_ROW(L, (*mpkInArray)[mpk_cur_offset]);                 \
    (*mpkOutArray)[mpk_next_offset][row] =                                     \
        _scale_ * ((*mpkOutArray)[mpk_next_offset][row] + tmp) __VA_ARGS__;    \
  }
...
#define BASE_PRECON_MPK_splitLU(_scale_, ...)                                  \
  for (LocalOrdinal row = start; row < end; ++row) {                           \
    BASE_SpMV_KERNEL_IN_ROW(L, (*mpkInArray)[mpk_cur_offset]);                 \
    (*mpkOutArray)[mpk_next_offset][row] =                                     \
        tmp + /*L->diag[row]*/ 1 * (*mpkInArray)[mpk_cur_offset][row];         \
  }                                                                            \
  for (LocalOrdinal row = start; row < end; ++row) {                           \
    BASE_SpMV_KERNEL_IN_ROW(U, (*mpkInArray)[mpk_cur_offset]);                 \
    (*mpkOutArray)[mpk_next_offset][row] =                                     \
...
#define BASE_Precon_PRECON(_precon_, ...)                                      \
  if (preconType == "TWO-STEP-GAUSS-SEIDEL") {                                 \
    ...
    for (LocalOrdinal row = start; row < end; ++row) {                         \
      BASE_PRECON_##_precon_##_KERNEL_IN_ROW(                                  \
          A, (*preconInArray)[precon_cur_offset],                              \
          (*preconOutArray)[precon_next_offset]);                              \
  ...
  {                                                                            \
    if (initVecIsZero) {                                                       \
      ...
      for (LocalOrdinal row = start; row < end; ++row) {                       \
        (*mpkOutArray)[mpk_next_offset][row] =                                 \
            (gamma * U->invDiag[row] *                                         \
             (*preconInArray)[precon_cur_offset][row]);                        \
      }                                                                        \
    } else {                                                                   \
      /*find residual*/                                                        \
      ...
      if (startSubPow == endSubPow) {                                          \
        ...
        BASE_PRECON_MPK_splitUL(-gamma,                                        \
                                +(gamma * U->invDiag[row] *                    \
                                  (*preconInArray)[precon_cur_offset][row]) +  \
                                    (*mpkInArray)[mpk_cur_offset][row]);       \
    ...
    if (initVecIsZero) /*Remember incase initVec is 0, then one less subPow is \
                          only needed, but this is not exploited now*/         \
    {                                                                          \
      ...
      if (subPow == (endSubPow)) {                                             \
        mpkOutArray = preconOutArray;                                          \
        mpk_next_offset = precon_next_offset;                                  \
        BASE_PRECON_MPK_lower(                                                 \
            -gamma, +(*mpkInArray)[1][row] +                                   \
                        gamma2 * (*mpkInArray)[mpk_cur_offset][row]);          \
      } else {                                                                 \
        BASE_PRECON_MPK_lower(                                                 \
            -gamma, +(*mpkInArray)[1][row] +                                   \
                        gamma2 * (*mpkInArray)[mpk_cur_offset][row]);          \
      ...
      if (subPow == (endSubPow)) {                                             \
        ...
        BASE_PRECON_MPK_lower(                                                 \
            -gamma, +(*mpkInArray)[1][row] +                                   \
                        gamma2 * (*mpkInArray)[mpk_cur_offset][row] +          \
                        (*mpkInArray)[0][row]); /*do update too*/              \
      } else {                                                                 \
        BASE_PRECON_MPK_lower(                                                 \
            -gamma, +(*mpkInArray)[1][row] +                                   \
                        gamma2 * (*mpkInArray)[mpk_cur_offset][row]);          \
...
#define GENERATE_KERNEL_Precon(_precon_)                                       \
  template <typename packtype>                                                 \
  inline void RACE_Precon_##_precon_##_KERNEL(                                 \
      int start, int end, int pow, int subPow, int numa_domain, void *args) {  \
...
// dispatcher function
template <typename packtype>
inline void RACE_Precon_KERNEL(int start, int end, int pow, int subPow,
                               int numa_domain, void *args) {
  using arg_type = kernelArgPrecon<packtype>;
  arg_type *arg_decode = (arg_type *)args;
  std::string preconType = arg_decode->preconType;

  // RACE_Precon_KERNEL<packtype>(std::integral_constant<bool,
  // packtype::STS::isComplex>{}, start, end, pow, subPow, numa_domain, args);
  if (preconType == "JACOBI") {
    RACE_Precon_JACOBI_KERNEL<packtype>(start, end, pow, subPow, numa_domain,
                                        args);
  } else if (preconType == "GAUSS-SEIDEL") {
    RACE_Precon_GAUSS_SEIDEL_KERNEL<packtype>(start, end, pow, subPow,
                                              numa_domain, args);
  } else if (preconType == "JACOBI-GAUSS-SEIDEL") {
    RACE_Precon_JACOBI_GAUSS_SEIDEL_KERNEL<packtype>(start, end, pow, subPow,
                                                     numa_domain, args);
  } else if ((preconType == "TWO-STEP-GAUSS-SEIDEL") ||
             (preconType == "NONE")) {
    RACE_Precon_NONE_KERNEL<packtype>(start, end, pow, subPow, numa_domain,
                                      args);
  } else {
