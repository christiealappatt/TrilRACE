...
struct CmdLineArgs {
  ...
  std::string polynomialType = "Roots";
  std::string polynomialTol = "1.0e-3";
  std::string polynomialDegree = "25";
...
getCmdLineArgs (CmdLineArgs& args, int argc, char* argv[])
{
  ...
  cmdp.setOption ("polyType", &args.polynomialType,
                  "Type of polynomial preconditioning,"
                  "options: Roots, Arnoldi, Gmres");
  cmdp.setOption ("polyTol", &args.polynomialTol,
                "Relative residual tolerance used to construct the GMRES polynomial");
  cmdp.setOption ("polyDeg", &args.polynomialDegree,
                "Maximum polynomial degree to construct the GMRES polynomial");
...
public:
  ...
  BelosIfpack2Solver (const Teuchos::RCP<CrsMatrixType>& A,
                      const std::string& solverType = "TPETRA HYBRID BLOCK GMRES",
                      const std::string& precType = "NONE") :
    A_ (A),
    solverType_ ("TPETRA HYBRID BLOCK GMRES"), //only this is allowed for this executable, outer solver can be chosen with solverType
    precType_ (precType),
    equilibrate_ (false),
...
private:
  ...
  //std::string polyPrecType_;
  //Teuchos::RCP<Teuchos::ParameterList> polyPrecParams_;
                ...
                int raceTunedPow=1)
{
  ...
  outerSolverParams->set("Max Orthogonalization Passes", 1);
...
*/
  ...
  if (args.useReorderingInIfpack2) {
    ...
    //  precParams->set ("relaxation: type", "MT Gauss-Seidel");
      ...
      precParams->set ("relaxation: zero starting solution", true);
...
/*
  std::string polyTypeValues;
  if (args.polynomialType == "") {
      polyTypeValues = "Roots";
  }
  else {
      polyTypeValues = args.polynomialType;
  }


  double polyToleranceValues;
  if (args.polynomialTol == "") {
      polyToleranceValues = 1e-3;
  }
  else {
      polyToleranceValues = atof(args.polynomialTol.c_str());
  }
*/
  ...
  //writer_type::writeSparseFile ("mtx_after_perm", A, "Matrix", "");
  ...
  if (args.rhsFilename == "") {
    RCP<MV> origB;
    origB = Teuchos::rcp (new MV (A->getRangeMap (), 1));
